...
public class ContactListAdapter extends
	 ...
	 */
	private ProgressBar connectedView;

	/**
	 * Image view with disconnected icon.
	 */
	// private ProgressBar disconnectedView;
	...
	public ContactListAdapter(ListActivity activity) {
		...
		if (infoView != null) {
			connectedView = (ProgressBar) infoView.findViewById(R.id.connected);
			// disconnectedView =
			// (ProgressBar)infoView.findViewById(R.id.disconnected);
	...
	public void onChange() {
		...
		if (filterString == null) {
			...
			for (RosterContact rosterContact : rosterContacts) {
				...
				sp = activity.getSharedPreferences(Constant.information,
						Context.MODE_PRIVATE);
				sp.edit().putString("chatAccount", account).commit();
		...
		if (infoView != null) {
			if (hasVisible) {
				...
				if (filterString != null) {
					...
					text = R.string.application_action_no_online;
					button = 0;
