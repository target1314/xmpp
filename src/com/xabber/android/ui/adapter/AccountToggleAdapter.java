...
public class AccountToggleAdapter implements UpdatableAdapter {
	...
	public void rebuild() {
		...
		while (linearLayout.getChildCount() < size) {
			...
			activity.registerForContextMenu(accountViewHolder.avatar);
			accountViewHolder.avatar.setOnClickListener(onClickListener);
	...
	public void onChange() {
		for (int index = 0; index < accounts.size(); index++) {
			...
			accountViewHolder.avatar.setImageDrawable(AvatarManager
					.getInstance().getUserAvatarForContactList(
