...
public class RoomChat extends AbstractChat {
	...
	@Override
	protected MessageItem newMessage(String text, String from) {
	...
	protected boolean onPacket(String bareAddress, Packet packet) {
		...
		if (packet instanceof Message) {
			...
			if (message.getType() == Message.Type.error) {
				...
				if (invite != null) {
					newAction(nickname, invite, ChatAction.invite_error, "OUT");
				}
				return true;
			}
			...
			if (subject != null) {
				...
				RosterManager.getInstance().onContactChanged(account,
						bareAddress);
				newAction(resource, subject, ChatAction.subject, "OUT");
			} else {
				...
				MessageItem messageItem = newMessage(resource, text, null,
						delay, true, true, false, false, true, "OUT");
	...
	private void onAvailable(String resource) {
		if (isSelf(resource)) {
			...
			if (isRequested()) {
				...
				if (showStatusChange())
					newAction(resource, null, ChatAction.complete, "OUT");
			}
		} else {
			if (state == RoomState.available)
				if (showStatusChange())
					newAction(resource, null, ChatAction.join, "OUT");
		}
	}
	...
	private void onLeave(String resource) {
		if (showStatusChange())
			newAction(resource, null, ChatAction.leave, "OUT");
		if (isSelf(resource)) {
	...
	private void onKick(String resource, String actor) {
		if (showStatusChange())
			newAction(resource, actor, ChatAction.kick, "OUT");
		if (isSelf(resource))
	...
	private void onBan(String resource, String actor) {
		if (showStatusChange())
			newAction(resource, actor, ChatAction.ban, "OUT");
		if (isSelf(resource))
			MUCManager.getInstance().leaveRoom(account, user);
	}
	...
	private void onRename(String resource, String newNick) {
		if (showStatusChange())
			newAction(resource, newNick, ChatAction.nickname, "OUT");
	}
