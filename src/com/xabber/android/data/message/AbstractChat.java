...
public abstract class AbstractChat extends BaseEntity {
	...
	private MessageItem createMessageItem(Cursor cursor) {
		MessageItem messageItem = new MessageItem(this,
				...
				true, false, false, MessageTable.getVoiceTime(cursor));
	 ...
	 */
	abstract protected MessageItem newMessage(String text, String from);
	...
	protected MessageItem newMessage(String resource, String text,
		...
		MessageItem messageItem = new MessageItem(this, record ? null
				...
				unencrypted, offline, "IN");
		messages.add(messageItem);
		updateSendQuery(messageItem);
		sort();
		if (save)
			requestToWriteMessage(messageItem, resource, text, action,
					timestamp, delayTimestamp, incoming, read, send, from);
		if (showTicker)
		...
		Application.getInstance().runInBackground(new Runnable() {
			...
			public void run() {
				long id = MessageTable.getInstance().add(account, user, null,
						...
						incoming, read, sent, false, voicetime);
		...
		for (final MessageItem messageItem : sendQuery) {
			...
			if (text == null) {
				...
				if (text.contains(".jpg") || text.contains(".png")
						|| text.contains(".gif") || text.contains(".jpeg")
						|| text.contains(".9.png")) {
					return;
				} else if (text.contains(".amr")) {
					return;
				} else if (text.contains(".message")) {
					String strf = new String(text);
					str_text = strf.split("\\.");
				} else if (text.contains(".location")) {
					String strf = new String(text);
					str_text = strf.split("\\.");
				}
				Message message = createMessagePacket(str_text[0]);
