...
public class NotificationManager implements OnInitializedListener,
	...
	public <T extends NotificationItem> void updateNotifications(
		...
		if (!iterator.hasNext()) {
			...
			String topName = new String(top.getTitle());
			String[] top_title = topName.split("@");
			Intent intent = top.getIntent();
			Notification notification = new Notification(provider.getIcon(),
					top_title[0], System.currentTimeMillis());
			if (!provider.canClearNotifications())
				notification.flags |= Notification.FLAG_NO_CLEAR;
			notification.setLatestEventInfo(application, top_title[0], top
	...
	private void updateMessageNotification(MessageItem ticker) {
		...
		if (messageNotifications.isEmpty()) {
			...
			if (Application.SDK_INT >= 14 && SettingsManager.eventsPersistent()) {
				...
				notification.icon = connected > 0 ? R.drawable.logo
						: R.drawable.logo;
		...
		persistentNotification.icon = R.drawable.logo;
		...
		if (Application.SDK_INT >= 14 && SettingsManager.eventsPersistent()) {
			...
			if (messageNotifications.isEmpty()) {
				persistentNotification.icon = connected > 0 ? R.drawable.logo
						: R.drawable.logo;
			} else {
				persistentNotification.icon = connected > 0 ? R.drawable.logo
						: R.drawable.logo;
	...
	private void notify(int id, Notification notification) {
		LogManager.i(this, "Notification: " + id + ", ticker: "
				...
				+ ", light: "
				+ (notification.defaults & Notification.DEFAULT_LIGHTS));
		System.out.println("Notification: " + id + ", ticker: "
				+ notification.tickerText + ", sound: " + notification.sound
				+ ", vibro: "
				+ (notification.defaults & Notification.DEFAULT_VIBRATE)
				+ ", light: "
	...
	private void updateNotification(Notification notification,
		...
		if (ticker.getChat().getFirstNotification()
			...
			String str = new String(ticker.getText());
			final String[] str_text = str.split("\\.");
			setNotificationDefaults(
					...
					PhraseManager.getInstance().getSound(
							ticker.getChat().getAccount(),
							ticker.getChat().getUser(), str_text[0]),
					AudioManager.STREAM_NOTIFICATION);
			if (ChatManager.getInstance().isShowText(
					ticker.getChat().getAccount(), ticker.getChat().getUser()))
				notification.tickerText = trimText(str_text[0]);
		}
	}
